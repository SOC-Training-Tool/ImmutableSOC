...
class SetupValidatorSpec extends AnyFunSpec with Matchers:
  ...
  describe("slot order"):
    ...
    it("only the first player has moves on an empty board"):
      ...
      val p0 = SetupValidator.legalMoves(0, 4, state.setupPlacementOrder, state.vertexBuildingState, state.edgeBuildingState, cached)
      val p1 = SetupValidator.legalMoves(1, 4, state.setupPlacementOrder, state.vertexBuildingState, state.edgeBuildingState, cached)
    ...
    it("reverses order for the second round"):
      ...
      val round2p3 = SetupValidator.legalMoves(3, 4, state.setupPlacementOrder, state.vertexBuildingState, state.edgeBuildingState, cached)
  ...
  describe("legality checks"):
    ...
    it("requires the road edge to be incident to the settlement vertex"):
      val moves = SetupValidator.legalMoves(0, 4, initPerfect.setupPlacementOrder, initPerfect.vertexBuildingState, initPerfect.edgeBuildingState, cached)
      moves.forall(m => cached.edgesFromVertex(m.vertex).contains(m.edge)) shouldBe true

    it("requires the road edge to be empty"):
      val state = initPerfect.copy(
        edgeBuildingState = EdgeBuildingState(Map(Edge(Vertex(40), Vertex(41)) -> PlayerBuilding(0, Road)))
      )
      val moves = SetupValidator.legalMoves(0, 4, state.setupPlacementOrder, state.vertexBuildingState, state.edgeBuildingState, cached)
    ...
    it("enforces the distance rule against adjacent settlements"):
      ...
      val moves = SetupValidator.legalMoves(1, 4, state.setupPlacementOrder, state.vertexBuildingState, state.edgeBuildingState, cached)
