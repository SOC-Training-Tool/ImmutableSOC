...
class RobberValidatorSpec extends AnyFunSpec with Matchers:
  ...
  describe("validHexes"):
    ...
    it("excludes the current robber hex"):
      val hexes = RobberValidator.validHexes(cached, RobberLocation(10))
  ...
  describe("stealTargets"):
    ...
    it("enumerates only opponents with cards on the hex"):
      ...
      val targets = RobberValidator.stealTargets(5, 0, cached, state.vertexBuildingState, inv)
    ...
    it("returns no targets when every victim has zero cards"):
      ...
      val inv = new PerfectInfoResourceView(state)
      RobberValidator.stealTargets(5, 0, cached, state.vertexBuildingState, inv) shouldBe empty

    it("returns no targets when no buildings sit on the hex"):
      val state = stateWith(Map.empty, Map.empty)
      val inv = new PerfectInfoResourceView(state)
      RobberValidator.stealTargets(11, 0, cached, state.vertexBuildingState, inv) shouldBe empty
  ...
  describe("placements and robberMoves"):
    ...
    it("yields a no-steal move for a hex with no victims"):
      val state = stateWith(Map.empty, Map.empty)
      val inv = new PerfectInfoResourceView(state)
      val placements = RobberValidator.placements(0, RobberLocation(10), cached, state.vertexBuildingState, inv)
    ...
    it("public robber moves hide the stolen resource"):
      ...
      val inv = new PerfectInfoResourceView(state)
      val moves = RobberValidator.robberMoves(0, RobberLocation(10), cached, state.vertexBuildingState, inv)
