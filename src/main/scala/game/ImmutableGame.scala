...
trait ImmutableGame[MOVES <: Coproduct, STATE <: HList] { self =>

  type GAME_STATE = STATE

  def applyMove(move: MOVES, state: STATE): STATE
...
object ImmutableGame {
  ...
  object MakeFolder extends Poly1 {
    implicit def onMove[M, STATE <: HList, S <: HList](implicit action: GAction.Aux[M, S], dep: DependsOn[STATE, S]): Case.Aux[(M, STATE), STATE] =
      at { case (move, state) => dep.updateAll(state)(action.apply(move, _)) }
  }

  final case class MakeApply[MOVE <: Coproduct, STATE <: HList]() {
    def apply[Z <: Coproduct, F <: shapeless.Poly]()(implicit z: coproduct.ZipConst.Aux[STATE, MOVE, Z], folder: coproduct.Folder.Aux[F, Z, STATE]): ImmutableGame[MOVE, STATE] = {
      (move: MOVE, state: STATE) => folder.apply(move.zipConst(state))
    }
  }

  final case class ApplyApply[MOVE <: Coproduct]() {
    def apply[M <: HList, STATE <: HList, Z <: Coproduct]()(implicit
                                                            toHlist: coproduct.ToHList.Aux[MOVE, M],
                                                            collect: utils2.CollectState.Aux[M, STATE],
                                                            z: coproduct.ZipConst.Aux[STATE, MOVE, Z],
                                                            folder: coproduct.Folder.Aux[MakeFolder.type, Z, STATE]
...
object utils2 {
  ...
  object CollectState {
    ...
    implicit def recur[H, T <: HList, S1 <: HList, S2 <: HList](implicit
     action: GAction.Aux[H, S1],
     next: Aux[T, S2],
     un: Union[S1, S2]
    ): Aux[H :: T, un.Out] = new CollectState[H :: T] {
      type Out = un.Out
    }
  ...
  object AddActions {
    ...
    object ComposeActionMapper extends Poly2 {
      ...
      implicit def atCase[A1, A2, M, S1 <: HList, S2 <: HList, SOut <: HList](implicit
    ...
    implicit def add[L <: HList, AL <: HList, ML <: HList, AML <: HList, Shared <: HList, R1 <: HList, R2 <: HList, O1 <: HList, O2 <: HList, O3 <: HList, O4 <: HList, Z <: HList, P1 <: HList](implicit
  ...
  object ApplyComposite {
    ...
    implicit def recur[H, T <: HList, M, A, B, S <: HList](implicit
  ...
  object StateHList {
    ...
    implicit def row[H, T <: HList, M, S <: HList, TOut <: HList, Out0 <: HList](implicit
     ev: H <:< GameAction[M, S],
  ...
  object MultiMoveAction {
    ...
    implicit def recur[H, T <: Coproduct, A, M, S <: HList, S0 <: HList, Out0 <: HList](implicit
     ev: A <:< GameAction[M, S],
