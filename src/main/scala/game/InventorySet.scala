...
object InventorySet {
  ...
  def toList[A](set: InventoryMap[A]): Seq[A]                               = set.amountMap.toList.flatMap { case (a, amt) => (1 to amt).map(_ => a) }
  ...
  def fromList[A](list: Seq[A]): InventoryMap[A]                            = fromMap(list.groupBy(f => f).view.mapValues(_.length).toMap)
  def sum[A, T: Numeric](sets: Seq[InventorySet[A, T]]): InventorySet[A, T] = sets.foldLeft(empty[A, T]) { case (x, y) => x.add(y) }
