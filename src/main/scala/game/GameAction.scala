...
trait GameAction[MOVE, S, DELTA] extends ((MOVE, S) => List[DELTA]) { self =>
...
trait GAction[M] {
  ...
  def apply(m: M, s: S): List[DL]
}

object GameAction {

  def apply[MOVE, S, DL](f: (MOVE, S) => List[DL]): GameAction[MOVE, S, DL] = (move: MOVE, state: S) => f(move, state)
}
...
object GAction {
  ...
  implicit def fromAction[M, SOut, DLOut](implicit gameAction: GameAction[M, SOut, DLOut]): Aux[M, SOut, DLOut] = new GAction[M] {
    ...
    override def apply(m: M, s: S): List[DL] = gameAction.apply(m, s)
  }
}

trait CollectActions[MOVES <: Coproduct] { type Out <: HList }

object CollectActions {

  type Aux[M <: Coproduct, Out0 <: HList] = CollectActions[M] { type Out = Out0 }

  def apply[M <: Coproduct](implicit collectActions: CollectActions[M]): Aux[M, collectActions.Out] = collectActions

  implicit def recur[H, S, DL, T <: Coproduct](implicit gaAction: GAction.Aux[H, S, DL], next: CollectActions[T]): Aux[H :+: T, GAction.Aux[H, S, DL] :: next.Out] = new CollectActions[H :+: T] {
    override type Out = GAction.Aux[H, S, DL] :: next.Out
  }

  implicit val hnil: Aux[CNil, HNil] = new CollectActions[CNil] { type Out = HNil}
}

trait CollectActionState[Actions <: HList] {
  type STATE <: HList
  type DELTAS <: Coproduct
}

object CollectActionState {

  type Aux[A <: HList, S <: HList, D <: Coproduct] = CollectActionState[A] {
    type STATE = S
    type DELTAS = D
  }

  def apply[AL <: HList](implicit c: CollectActionState[AL]): Aux[AL, c.STATE, c.DELTAS] = c

  implicit def recur[M, S <: HList, D <: Coproduct, T <: HList](implicit next: CollectActionState[T]): Aux[GAction.Aux[M, S, D] :: T, ]
