...
case class PortTradeMove[II](player: Int, give: InventorySet[II, Int], get: InventorySet[II, Int])
    extends PerfectInformationGameMove[PortTradeMove[II]]

case class RobberMove[Res](player: Int, robberHexId: Int, victim: Option[Int]) extends GameMove

case class RobberMoveResult[Res](player: Int, robberHexId: Int, steal: Option[PlayerSteal[Option[Res]]])
    extends GameMoveResult:
  override type A = RobberMove[Res]
  override def move: RobberMove[Res] = RobberMove(player, robberHexId, steal.map(_.victim))

case class PerfectInfoRobberMoveResult[Res](
    player: Int, robberHexId: Int, steal: Option[PlayerSteal[Res]])
    extends PerfectInfoMoveResult:
  override type A                    = RobberMove[Res]
  override type PublicInfoMoveResult = RobberMoveResult[Res]
  override def move: A = RobberMove(player, robberHexId, steal.map(_.victim))
  override def getPerspectiveResults(playerIds: Seq[Int]): Map[Int, RobberMoveResult[Res]] =
    playerIds.map { p =>
...
case class PerfectInfoBuyDevelopmentCardMoveResult[Card](player: Int, card: Card)
    extends PerfectInfoMoveResult:
  override type A                    = BuyDevelopmentCardMove[Card]
  override type PublicInfoMoveResult = BuyDevelopmentCardMoveResult[Card]
...
case class PerfectInfoPlayKnightResult[Res](inner: PerfectInfoRobberMoveResult[Res])
    extends PerfectInfoMoveResult:
  override type A                    = PlayKnightMove[Res]
  override type PublicInfoMoveResult = PlayKnightResult[Res]
...
case class PlayMonopolyMoveResult[Res](player: Int, res: Res, cardsLost: Map[Int, Int])
    extends PerfectInfoMoveResult:
  override type A                    = PlayMonopolyMove[Res]
  override type PublicInfoMoveResult = PlayMonopolyMoveResult[Res]
  override def getPerspectiveResults(playerIds: Seq[Int]): Map[Int, PlayMonopolyMoveResult[Res]] =
    playerIds.map(_ -> this).toMap
...
case class PlayRoadBuilderMove(player: Int, edge1: Edge, edge2: Option[Edge])
    extends PerfectInformationGameMove[PlayRoadBuilderMove]

case class PlayYearOfPlentyMove[Res](player: Int, c1: Res, c2: Res)
    extends PerfectInformationGameMove[PlayYearOfPlentyMove[Res]]
